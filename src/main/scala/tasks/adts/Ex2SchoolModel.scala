...
object SchoolModel:
    ...
    object BasicSchoolModule extends SchoolModule:
        ...
        extension (school: School)
            def courses: Sequence[String] =
                def _courses(s: School): Sequence[String] = s match
                    case Cons((_, c), t) => cons(c, _courses(t))
                    case _ => Nil()
            
                _courses(school).distinct()
                
            def teachers: Sequence[String] =
                def _teachers(s: School): Sequence[String] = s match
                    case Cons((te, _), t) => cons(te, _teachers(t))
                    case _ => Nil()
                _teachers(school).distinct()
            ...
            def coursesOfATeacher(teacher: Teacher): Sequence[Course] = filter(school)((t, c) => t == teacher).map((t, c) => c)
            
            def hasTeacher(name: String): Boolean = school match
                case Cons((te, _), t) => te == name || t.hasTeacher(name)
                case _ => false
            
            def hasCourse(name: String): Boolean = school match
                case Cons((_, c), t) => c == name || t.hasCourse(name)
                case _ => false
